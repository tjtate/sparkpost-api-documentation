...
**Warning:** Before deleting a sending domain please ensure you are no longer using it. After deleting a sending domain any new transmissions that use it will result in a rejection, including any transmissions that are in progress, transmissions scheduled for the future or that use a stored template referencing the sending domain.
