...
* When using 'basic' auth, auth_credentials must be set by the user and should be an object containing "user_id" and "password" (example: `{ "user_id": "basicauthuser", "password": "mypassword" }`)
...
| auth_credentials         | JSON | Object containing credentials needed to make authorized POST requests to target | no | examples: `{ "access_token": "<oauth token>", expires_in: 3600 }`, `{ "user_id": "basicauthuser", "password": "mypassword" }` |
