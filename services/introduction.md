...
* To send mail over an SMTP relay endpoint as a Subaccount, modify the your `User` to include the subaccount ID.
  * For example, use `SMTP_Injection:X-MSYS-SUBACCOUNT=123` to send as a Subacount, having an ID of 123.  The Master Account's API key is still used as the password in this case.
